@php use App\Enums\SiteSettings; @endphp
@props(['post'])

<div {{ $attributes->class('flex flex-col h-full') }}>
    <a wire:navigate href="{{ route('posts.show', ['post' => $post]) }}"> <img src="{{ $post->image_url }}"
                                                                               alt="{{ $post->title  }}"
                                                                               class="object-cover rounded-xl ring-1 shadow-md transition-opacity shadow-black/5 dark:shadow-white/5 aspect-video hover:opacity-50 ring-black/5 dark:ring-white/5"/>
    </a>
    ...
    <div class="flex gap-6 justify-between items-center mt-5">
        <a wire:navigate href="{{ route('posts.show', ['post' => $post]) }}"
           class="font-bold transition-colors text-xl/tight hover:text-primary-600 dark:hover:text-primary-400">
            {{ $post->title }}
        </a>
