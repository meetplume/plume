...
@if(isset($variant) && $variant === 'card')

    <a {{ $attributes->class('grid grid-cols-1 gap-4 items-center') }} wire:navigate href="{{ route('posts.show', ['post' => $post]) }}">
...
@elseif(isset($variant) && $variant === 'mini')
    ...
    <a {{ $attributes->class('grid grid-cols-1 gap-4 items-center') }} wire:navigate href="{{ route('posts.show', ['post' => $post]) }}">
        ...
        <div class="col-span-full">
            <h3 class="font-bold text-lg text-white mb-4 md:text-lg lg:text-xl">
                {{ $post->title }}
            </h3>
            <div class="">
                {!! $post->description ?? '' !!}
